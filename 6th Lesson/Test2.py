...
class Ball:
    ...
    # обрабатываем касание платформы, для этого получаем 4 координаты шарика в переменной pos (левая верхняя и правая
    # нижняя точки)
...
class Score:
    ...
    def __init__(self, canvas, color):
        ...
        self.canvas = canvas
        # создаём надпись, которая показывает текущий счёт, делаем его нужно цвета и запоминаем внутреннее имя этой
        # надписи
...
while not ball_1.hit_bottom:
    ...
    speed = score.score * 0.00025
    if speed > 0.009: speed = 0.009
    time.sleep(0.01 - speed)
# если программа дошла досюда, значит, шарик коснулся дна. Ждём 3 секунды, пока игрок прочитает финальную надпись,
# и завершаем игру
time.sleep(3)
